...
static inline QImage mat_to_qimage(const Mat &mat) {
    ...
    } else if (mat2.type() == CV_32FC3 || mat2.type() == CV_64FC3) {
        Mat temp;
        mat2.convertTo(temp, CV_8UC3, 255.0, 0.0);
        return QImage(temp.data, temp.cols, temp.rows, temp.step, QImage::Format_RGB888).rgbSwapped().copy();
