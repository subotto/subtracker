...
def analyze_table_background(bg_estimation, warped_frame, controls):
    ...
    deviation = (warped_frame - bg_estimation.mean)

    bg_sigma = controls.trackbar("sigma", 0.2, 0.01, 0.5, 0.01).value
    analysis.background_ll = numpy.sum(deviation * deviation, 2) * (-0.5 / bg_sigma ** 2)
    ...
    ball_deviation = warped_frame - ball_intensity
    analysis.ball_ll = numpy.sum(ball_deviation * ball_deviation, 2) * (-0.5 / ball_sigma ** 2)
